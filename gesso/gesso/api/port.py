...
class Port(object):
    ...
    def get_compatible_components(self):
        return self.__compatible_components
                ...
                for state in port.states:
                    ...
                    for this_state in self.states:
                        for compatible_state in this_state.get_compatible_states():
                            if State.compare(state, compatible_state):
                                # Just-in-time initialize storage for compatible port states on other component's port to empty list
                                if component not in self.__compatible_components:
                                    self.__compatible_components.append(component)
                                if port not in self.__compatible_ports:
                                    self.__compatible_ports.append(port)
                                if state not in self.__compatible_states:
                                    self.__compatible_states.append(state)

	return (self.__compatible_components, self.__compatible_ports, self.__compatible_states)
