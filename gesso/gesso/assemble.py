...
def assemble(component_identifiers):
	...
	logger.info('%s' % components)
        ...
        automatically_compose = raw_input("Automatically compose the components? (Y/n):")
...
def find_component_files(component_identifiers):
		...
		# Check for GitHub repository identifier
                if component_identifier not in component_file_paths:
                    ...
                    if component_github_pattern.match(component_identifier) is not None:
                            logger.info('Cloning %s/%s to %s/%s/%s' % (username, repository, '.gesso/components', username, repository))
	...
	for component_identifier in component_identifiers:
		logger.info('\t%s => Found component file: %s' % (component_identifier, component_file_paths[component_identifier]))
	logger.info('')
...
def assemble_components(components):
	...
	hosts = []
        for component in system.components:
            # if component.name == 'Raspberry Pi 3':
            if component.host:
                hosts.append(component)

        # System.compose(list_of_systems)
        # system = Component.compose(list_of_components)
	...
	for component in system.components:
		...
		logger.info("Let's connect the <%s>." % component.name)
		for port in component.ports:
			# TODO: search for compatible device (first look for power source (prioritized dependency satisfaction), then verify other dependencies, perserving interface consistency for multi-port interfaces)
                        ...
                        for compatible_state in compatible_state_list:
                            logger.info("\t\t\t%s: mode: %s, direction: %s, voltage: %s" % (compatible_state.port.number, compatible_state.mode, compatible_state.direction, compatible_state.voltage))

                            if compatible_state.port not in occupied_ports and port not in paths[component]:
                                occupied_ports.append(compatible_state.port)
                                paths[component][port] = { 'port': compatible_state.port, 'state': compatible_state }
	...
	for source_component in paths.keys():
                print 'Connect the component: %s:' % source_component.name
                for source_port in paths[source_component].keys():
                    print '\tConnect port %s on %s to port %s on %s' % (source_port.number, source_component.name, paths[source_component][source_port]['port'].number, paths[source_component][source_port]['port'].component.name)
